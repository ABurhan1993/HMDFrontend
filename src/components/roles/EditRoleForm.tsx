...
import type { RoleData } from "@/pages/Roles/RoleList";
...
export default function EditRoleForm({ editingRole, availablePermissions, onClose, onSuccess }: Props) {
  const [roleName, setRoleName] = useState(editingRole.name);
  const [selectedPermissions, setSelectedPermissions] = useState<string[]>(editingRole.claims || []);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-y-auto">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-md w-full">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-4 text-center">Edit Role</h2>
